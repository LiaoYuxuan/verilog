-- Copyright 1986-2017 Xilinx, Inc. All Rights Reserved.
-- --------------------------------------------------------------------------------
-- Tool Version: Vivado v.2017.1 (win64) Build 1846317 Fri Apr 14 18:55:03 MDT 2017
-- Date        : Sun Jul  8 13:44:28 2018
-- Host        : LAPTOP-KM039FVS running 64-bit major release  (build 9200)
-- Command     : write_vhdl -force -mode funcsim -rename_top decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix -prefix
--               decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_ IICctrl_0_sim_netlist.vhdl
-- Design      : IICctrl_0
-- Purpose     : This VHDL netlist is a functional simulation representation of the design and should not be modified or
--               synthesized. This netlist cannot be used for SDF annotated simulation.
-- Device      : xc7a35tcpg236-1
-- --------------------------------------------------------------------------------
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_I2C_Controller is
  port (
    E : out STD_LOGIC_VECTOR ( 0 to 0 );
    \SD_COUNTER_reg[0]_0\ : out STD_LOGIC;
    D : out STD_LOGIC_VECTOR ( 1 downto 0 );
    I2C_SCLK : out STD_LOGIC;
    I2C_SDAT : inout STD_LOGIC;
    iCLK : in STD_LOGIC;
    rst : in STD_LOGIC;
    mI2C_WR_reg : in STD_LOGIC;
    mI2C_GO_reg : in STD_LOGIC;
    Q : in STD_LOGIC_VECTOR ( 1 downto 0 );
    \LUT_INDEX_reg[5]\ : in STD_LOGIC;
    i2c_en_r1 : in STD_LOGIC;
    i2c_en_r0 : in STD_LOGIC;
    Slave_Addr : in STD_LOGIC_VECTOR ( 7 downto 0 );
    LUT_DATA : in STD_LOGIC_VECTOR ( 15 downto 0 );
    \LUT_INDEX_reg[7]\ : in STD_LOGIC_VECTOR ( 6 downto 0 );
    \LUT_INDEX_reg[5]_0\ : in STD_LOGIC;
    mI2C_CTRL_CLK : in STD_LOGIC
  );
end decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_I2C_Controller;

architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_I2C_Controller is
  signal ACKR13_out : STD_LOGIC;
  signal ACKR1_i_1_n_0 : STD_LOGIC;
  signal ACKR1_i_3_n_0 : STD_LOGIC;
  signal ACKR1_reg_n_0 : STD_LOGIC;
  signal ACKR25_out : STD_LOGIC;
  signal ACKR2_i_1_n_0 : STD_LOGIC;
  signal ACKR2_i_3_n_0 : STD_LOGIC;
  signal ACKR2_i_4_n_0 : STD_LOGIC;
  signal ACKR2_reg_n_0 : STD_LOGIC;
  signal ACKR34_out : STD_LOGIC;
  signal ACKR3_i_1_n_0 : STD_LOGIC;
  signal ACKR3_i_3_n_0 : STD_LOGIC;
  signal ACKR3_i_4_n_0 : STD_LOGIC;
  signal ACKR3_reg_n_0 : STD_LOGIC;
  signal ACKW10_out : STD_LOGIC;
  signal ACKW1_i_1_n_0 : STD_LOGIC;
  signal ACKW1_reg_n_0 : STD_LOGIC;
  signal ACKW22_out : STD_LOGIC;
  signal ACKW2_i_1_n_0 : STD_LOGIC;
  signal ACKW2_i_3_n_0 : STD_LOGIC;
  signal ACKW2_reg_n_0 : STD_LOGIC;
  signal ACKW31_out : STD_LOGIC;
  signal ACKW3_i_1_n_0 : STD_LOGIC;
  signal ACKW3_i_3_n_0 : STD_LOGIC;
  signal ACKW3_reg_n_0 : STD_LOGIC;
  signal END10_out : STD_LOGIC;
  signal END_i_1_n_0 : STD_LOGIC;
  signal END_i_3_n_0 : STD_LOGIC;
  signal END_i_4_n_0 : STD_LOGIC;
  signal END_i_5_n_0 : STD_LOGIC;
  signal I2C_BIT6_out : STD_LOGIC;
  signal I2C_BIT_i_10_n_0 : STD_LOGIC;
  signal I2C_BIT_i_11_n_0 : STD_LOGIC;
  signal I2C_BIT_i_12_n_0 : STD_LOGIC;
  signal I2C_BIT_i_13_n_0 : STD_LOGIC;
  signal I2C_BIT_i_14_n_0 : STD_LOGIC;
  signal I2C_BIT_i_17_n_0 : STD_LOGIC;
  signal I2C_BIT_i_18_n_0 : STD_LOGIC;
  signal I2C_BIT_i_19_n_0 : STD_LOGIC;
  signal I2C_BIT_i_1_n_0 : STD_LOGIC;
  signal I2C_BIT_i_20_n_0 : STD_LOGIC;
  signal I2C_BIT_i_22_n_0 : STD_LOGIC;
  signal I2C_BIT_i_23_n_0 : STD_LOGIC;
  signal I2C_BIT_i_24_n_0 : STD_LOGIC;
  signal I2C_BIT_i_25_n_0 : STD_LOGIC;
  signal I2C_BIT_i_26_n_0 : STD_LOGIC;
  signal I2C_BIT_i_27_n_0 : STD_LOGIC;
  signal I2C_BIT_i_28_n_0 : STD_LOGIC;
  signal I2C_BIT_i_29_n_0 : STD_LOGIC;
  signal I2C_BIT_i_2_n_0 : STD_LOGIC;
  signal I2C_BIT_i_30_n_0 : STD_LOGIC;
  signal I2C_BIT_i_31_n_0 : STD_LOGIC;
  signal I2C_BIT_i_32_n_0 : STD_LOGIC;
  signal I2C_BIT_i_33_n_0 : STD_LOGIC;
  signal I2C_BIT_i_34_n_0 : STD_LOGIC;
  signal I2C_BIT_i_35_n_0 : STD_LOGIC;
  signal I2C_BIT_i_36_n_0 : STD_LOGIC;
  signal I2C_BIT_i_37_n_0 : STD_LOGIC;
  signal I2C_BIT_i_5_n_0 : STD_LOGIC;
  signal I2C_BIT_i_6_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_15_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_16_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_21_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_3_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_7_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_8_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_i_9_n_0 : STD_LOGIC;
  signal I2C_BIT_reg_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_1_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_2_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_3_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_4_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_5_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_6_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_7_n_0 : STD_LOGIC;
  signal I2C_SCLK_INST_0_i_8_n_0 : STD_LOGIC;
  signal I2C_SDAT_INST_0_i_2_n_0 : STD_LOGIC;
  signal I2C_SDAT_INST_0_i_3_n_0 : STD_LOGIC;
  signal SCLK_i_1_n_0 : STD_LOGIC;
  signal SCLK_i_2_n_0 : STD_LOGIC;
  signal SCLK_i_3_n_0 : STD_LOGIC;
  signal SCLK_i_4_n_0 : STD_LOGIC;
  signal SCLK_i_5_n_0 : STD_LOGIC;
  signal SCLK_i_6_n_0 : STD_LOGIC;
  signal SCLK_i_7_n_0 : STD_LOGIC;
  signal SCLK_i_8_n_0 : STD_LOGIC;
  signal SCLK_i_9_n_0 : STD_LOGIC;
  signal SCLK_reg_n_0 : STD_LOGIC;
  signal SDO : STD_LOGIC;
  signal SD_COUNTER : STD_LOGIC;
  signal \SD_COUNTER[4]_i_2_n_0\ : STD_LOGIC;
  signal \SD_COUNTER[5]_i_3_n_0\ : STD_LOGIC;
  signal \SD_COUNTER[5]_i_4_n_0\ : STD_LOGIC;
  signal \^sd_counter_reg[0]_0\ : STD_LOGIC;
  signal \SD_COUNTER_reg__0\ : STD_LOGIC_VECTOR ( 5 downto 0 );
  signal g0_b0_n_0 : STD_LOGIC;
  signal mI2C_END : STD_LOGIC;
  signal \mSetup_ST[1]_i_2_n_0\ : STD_LOGIC;
  signal \mSetup_ST[1]_i_3_n_0\ : STD_LOGIC;
  signal \mSetup_ST[1]_i_4_n_0\ : STD_LOGIC;
  signal \p_0_in__0\ : STD_LOGIC_VECTOR ( 5 downto 0 );
  attribute SOFT_HLUTNM : string;
  attribute SOFT_HLUTNM of ACKW3_i_2 : label is "soft_lutpair2";
  attribute SOFT_HLUTNM of I2C_BIT_i_27 : label is "soft_lutpair3";
  attribute SOFT_HLUTNM of I2C_BIT_i_33 : label is "soft_lutpair3";
  attribute SOFT_HLUTNM of I2C_BIT_i_35 : label is "soft_lutpair4";
  attribute SOFT_HLUTNM of I2C_BIT_i_6 : label is "soft_lutpair0";
  attribute SOFT_HLUTNM of \LUT_INDEX[7]_i_4\ : label is "soft_lutpair2";
  attribute SOFT_HLUTNM of SCLK_i_5 : label is "soft_lutpair0";
  attribute SOFT_HLUTNM of \SD_COUNTER[1]_i_1\ : label is "soft_lutpair1";
  attribute SOFT_HLUTNM of \SD_COUNTER[2]_i_1\ : label is "soft_lutpair1";
  attribute SOFT_HLUTNM of \SD_COUNTER[4]_i_2\ : label is "soft_lutpair4";
begin
  \SD_COUNTER_reg[0]_0\ <= \^sd_counter_reg[0]_0\;
ACKR1_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FBFFFFFFFBFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => \SD_COUNTER_reg__0\(3),
      I2 => mI2C_WR_reg,
      I3 => mI2C_GO_reg,
      I4 => ACKR13_out,
      I5 => ACKR1_reg_n_0,
      O => ACKR1_i_1_n_0
    );
ACKR1_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"4540FFFF45400000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(4),
      I1 => ACKR3_i_3_n_0,
      I2 => mI2C_WR_reg,
      I3 => ACKR1_i_3_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => ACKR13_out
    );
ACKR1_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000040040004"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(1),
      I1 => \^sd_counter_reg[0]_0\,
      I2 => \SD_COUNTER_reg__0\(3),
      I3 => \SD_COUNTER_reg__0\(2),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKR1_i_3_n_0
    );
ACKR1_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKR1_i_1_n_0,
      PRE => rst,
      Q => ACKR1_reg_n_0
    );
ACKR2_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EFFFFFFFEFFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => mI2C_WR_reg,
      I2 => mI2C_GO_reg,
      I3 => \SD_COUNTER_reg__0\(4),
      I4 => ACKR25_out,
      I5 => ACKR2_reg_n_0,
      O => ACKR2_i_1_n_0
    );
ACKR2_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"2F20FFFF2F200000"
    )
        port map (
      I0 => ACKR2_i_3_n_0,
      I1 => mI2C_WR_reg,
      I2 => \SD_COUNTER_reg__0\(4),
      I3 => ACKR2_i_4_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => ACKR25_out
    );
ACKR2_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000000001000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \^sd_counter_reg[0]_0\,
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKR2_i_3_n_0
    );
ACKR2_i_4: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000000000700"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => mI2C_WR_reg,
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => END_i_5_n_0,
      I4 => \SD_COUNTER_reg__0\(1),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKR2_i_4_n_0
    );
ACKR2_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKR2_i_1_n_0,
      PRE => rst,
      Q => ACKR2_reg_n_0
    );
ACKR3_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FBFFFFFFFBFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => mI2C_WR_reg,
      I3 => mI2C_GO_reg,
      I4 => ACKR34_out,
      I5 => ACKR3_reg_n_0,
      O => ACKR3_i_1_n_0
    );
ACKR3_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"4540FFFF45400000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(4),
      I1 => ACKR3_i_3_n_0,
      I2 => mI2C_WR_reg,
      I3 => ACKR3_i_4_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => ACKR34_out
    );
ACKR3_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000000000010"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \^sd_counter_reg[0]_0\,
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKR3_i_3_n_0
    );
ACKR3_i_4: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000008000300"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \^sd_counter_reg[0]_0\,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => \SD_COUNTER_reg__0\(2),
      O => ACKR3_i_4_n_0
    );
ACKR3_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKR3_i_1_n_0,
      PRE => rst,
      Q => ACKR3_reg_n_0
    );
ACKW1_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"BFFFFFFFBFFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => \SD_COUNTER_reg__0\(3),
      I2 => mI2C_WR_reg,
      I3 => mI2C_GO_reg,
      I4 => ACKW10_out,
      I5 => ACKW1_reg_n_0,
      O => ACKW1_i_1_n_0
    );
ACKW1_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"4540FFFF45400000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(4),
      I1 => ACKR1_i_3_n_0,
      I2 => mI2C_WR_reg,
      I3 => ACKR3_i_3_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => ACKW10_out
    );
ACKW1_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKW1_i_1_n_0,
      PRE => rst,
      Q => ACKW1_reg_n_0
    );
ACKW2_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"BFFFFFFFBFFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => mI2C_WR_reg,
      I2 => mI2C_GO_reg,
      I3 => \SD_COUNTER_reg__0\(4),
      I4 => ACKW22_out,
      I5 => ACKW2_reg_n_0,
      O => ACKW2_i_1_n_0
    );
ACKW2_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"8F80FFFF8F800000"
    )
        port map (
      I0 => ACKR2_i_3_n_0,
      I1 => mI2C_WR_reg,
      I2 => \SD_COUNTER_reg__0\(4),
      I3 => ACKW2_i_3_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => ACKW22_out
    );
ACKW2_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000002000300"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => END_i_5_n_0,
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKW2_i_3_n_0
    );
ACKW2_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKW2_i_1_n_0,
      PRE => rst,
      Q => ACKW2_reg_n_0
    );
ACKW3_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"BFFFFFFFBFFF0000"
    )
        port map (
      I0 => I2C_SDAT,
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => mI2C_WR_reg,
      I3 => mI2C_GO_reg,
      I4 => ACKW31_out,
      I5 => ACKW3_reg_n_0,
      O => ACKW3_i_1_n_0
    );
ACKW3_i_2: unisim.vcomponents.LUT5
    generic map(
      INIT => X"40404F40"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(4),
      I1 => ACKW3_i_3_n_0,
      I2 => mI2C_GO_reg,
      I3 => i2c_en_r1,
      I4 => i2c_en_r0,
      O => ACKW31_out
    );
ACKW3_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"2000000002000300"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => END_i_5_n_0,
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => ACKW3_i_3_n_0
    );
ACKW3_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => ACKW3_i_1_n_0,
      PRE => rst,
      Q => ACKW3_reg_n_0
    );
END_i_1: unisim.vcomponents.LUT4
    generic map(
      INIT => X"8F80"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => mI2C_GO_reg,
      I2 => END10_out,
      I3 => mI2C_END,
      O => END_i_1_n_0
    );
END_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"2F20FFFF2F200000"
    )
        port map (
      I0 => END_i_3_n_0,
      I1 => mI2C_WR_reg,
      I2 => \SD_COUNTER_reg__0\(4),
      I3 => END_i_4_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => END10_out
    );
END_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"1000000000000000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \^sd_counter_reg[0]_0\,
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(5),
      O => END_i_3_n_0
    );
END_i_4: unisim.vcomponents.LUT6
    generic map(
      INIT => X"8000000000000F00"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => mI2C_WR_reg,
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => END_i_5_n_0,
      I4 => \SD_COUNTER_reg__0\(1),
      I5 => \SD_COUNTER_reg__0\(5),
      O => END_i_4_n_0
    );
END_i_5: unisim.vcomponents.LUT3
    generic map(
      INIT => X"04"
    )
        port map (
      I0 => i2c_en_r0,
      I1 => i2c_en_r1,
      I2 => \SD_COUNTER_reg__0\(3),
      O => END_i_5_n_0
    );
END_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => END_i_1_n_0,
      Q => mI2C_END
    );
I2C_BIT_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"B8BBFFFFB8BB0000"
    )
        port map (
      I0 => I2C_BIT_i_2_n_0,
      I1 => \SD_COUNTER_reg__0\(4),
      I2 => I2C_BIT_reg_i_3_n_0,
      I3 => mI2C_GO_reg,
      I4 => I2C_BIT6_out,
      I5 => I2C_BIT_reg_n_0,
      O => I2C_BIT_i_1_n_0
    );
I2C_BIT_i_10: unisim.vcomponents.LUT6
    generic map(
      INIT => X"88008800B8FF8800"
    )
        port map (
      I0 => I2C_BIT_reg_i_21_n_0,
      I1 => mI2C_WR_reg,
      I2 => g0_b0_n_0,
      I3 => mI2C_GO_reg,
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => I2C_BIT_i_10_n_0
    );
I2C_BIT_i_11: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EFE0FFFF40400000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => I2C_BIT_i_22_n_0,
      I2 => mI2C_WR_reg,
      I3 => g0_b0_n_0,
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => I2C_BIT_i_11_n_0
    );
I2C_BIT_i_12: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00FF00FF30BB3088"
    )
        port map (
      I0 => LUT_DATA(9),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => LUT_DATA(8),
      I3 => \SD_COUNTER_reg__0\(1),
      I4 => LUT_DATA(10),
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_12_n_0
    );
I2C_BIT_i_13: unisim.vcomponents.LUT4
    generic map(
      INIT => X"00B8"
    )
        port map (
      I0 => LUT_DATA(11),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => LUT_DATA(13),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_13_n_0
    );
I2C_BIT_i_14: unisim.vcomponents.LUT4
    generic map(
      INIT => X"00B8"
    )
        port map (
      I0 => LUT_DATA(12),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => LUT_DATA(14),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_14_n_0
    );
I2C_BIT_i_17: unisim.vcomponents.LUT6
    generic map(
      INIT => X"AFA0CFCFAFA0C0C0"
    )
        port map (
      I0 => I2C_BIT_i_27_n_0,
      I1 => I2C_BIT_i_28_n_0,
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => I2C_BIT_i_29_n_0,
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => I2C_BIT_i_30_n_0,
      O => I2C_BIT_i_17_n_0
    );
I2C_BIT_i_18: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B8BBB888"
    )
        port map (
      I0 => I2C_BIT_i_31_n_0,
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => I2C_BIT_i_32_n_0,
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => I2C_BIT_i_28_n_0,
      O => I2C_BIT_i_18_n_0
    );
I2C_BIT_i_19: unisim.vcomponents.LUT6
    generic map(
      INIT => X"AFA0CFCFAFA0C0C0"
    )
        port map (
      I0 => I2C_BIT_i_33_n_0,
      I1 => I2C_BIT_i_28_n_0,
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => I2C_BIT_i_34_n_0,
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => I2C_BIT_i_30_n_0,
      O => I2C_BIT_i_19_n_0
    );
I2C_BIT_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FFE2CCE2FFFFFFFF"
    )
        port map (
      I0 => I2C_BIT_i_5_n_0,
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => I2C_BIT_i_6_n_0,
      I3 => mI2C_WR_reg,
      I4 => I2C_BIT_reg_i_7_n_0,
      I5 => mI2C_GO_reg,
      O => I2C_BIT_i_2_n_0
    );
I2C_BIT_i_20: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EFEFEFEFEA45EA40"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => I2C_BIT_i_35_n_0,
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => \SD_COUNTER_reg__0\(1),
      I4 => LUT_DATA(1),
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_20_n_0
    );
I2C_BIT_i_22: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00FE000000FF0000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => i2c_en_r0,
      I4 => i2c_en_r1,
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_22_n_0
    );
I2C_BIT_i_23: unisim.vcomponents.LUT6
    generic map(
      INIT => X"AFCFAFCFAFC0AAC0"
    )
        port map (
      I0 => LUT_DATA(7),
      I1 => LUT_DATA(12),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => LUT_DATA(5),
      I5 => LUT_DATA(14),
      O => I2C_BIT_i_23_n_0
    );
I2C_BIT_i_24: unisim.vcomponents.LUT5
    generic map(
      INIT => X"A0A0CFC0"
    )
        port map (
      I0 => LUT_DATA(6),
      I1 => LUT_DATA(11),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => LUT_DATA(13),
      I4 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_24_n_0
    );
I2C_BIT_i_25: unisim.vcomponents.LUT6
    generic map(
      INIT => X"AFA0AFA0CFCFC0C0"
    )
        port map (
      I0 => LUT_DATA(3),
      I1 => LUT_DATA(8),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => LUT_DATA(5),
      I4 => LUT_DATA(10),
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_25_n_0
    );
I2C_BIT_i_26: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B833B800"
    )
        port map (
      I0 => LUT_DATA(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => LUT_DATA(4),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => LUT_DATA(9),
      O => I2C_BIT_i_26_n_0
    );
I2C_BIT_i_27: unisim.vcomponents.LUT5
    generic map(
      INIT => X"A0A0CFC0"
    )
        port map (
      I0 => LUT_DATA(15),
      I1 => Slave_Addr(4),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => Slave_Addr(6),
      I4 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_27_n_0
    );
I2C_BIT_i_28: unisim.vcomponents.LUT4
    generic map(
      INIT => X"00B8"
    )
        port map (
      I0 => Slave_Addr(5),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => Slave_Addr(7),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_28_n_0
    );
I2C_BIT_i_29: unisim.vcomponents.LUT4
    generic map(
      INIT => X"B833"
    )
        port map (
      I0 => Slave_Addr(0),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => Slave_Addr(2),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_29_n_0
    );
I2C_BIT_i_30: unisim.vcomponents.LUT4
    generic map(
      INIT => X"B833"
    )
        port map (
      I0 => Slave_Addr(1),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => Slave_Addr(3),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_30_n_0
    );
I2C_BIT_i_31: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00000000FCBBFC88"
    )
        port map (
      I0 => Slave_Addr(2),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => Slave_Addr(1),
      I3 => \SD_COUNTER_reg__0\(1),
      I4 => Slave_Addr(3),
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_31_n_0
    );
I2C_BIT_i_32: unisim.vcomponents.LUT4
    generic map(
      INIT => X"00B8"
    )
        port map (
      I0 => Slave_Addr(4),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => Slave_Addr(6),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_32_n_0
    );
I2C_BIT_i_33: unisim.vcomponents.LUT5
    generic map(
      INIT => X"F3BBF088"
    )
        port map (
      I0 => Slave_Addr(4),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => LUT_DATA(15),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => Slave_Addr(6),
      O => I2C_BIT_i_33_n_0
    );
I2C_BIT_i_34: unisim.vcomponents.LUT6
    generic map(
      INIT => X"F0FFF000FEFFFEFF"
    )
        port map (
      I0 => LUT_DATA(0),
      I1 => Slave_Addr(4),
      I2 => Slave_Addr(0),
      I3 => \SD_COUNTER_reg__0\(1),
      I4 => Slave_Addr(2),
      I5 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_34_n_0
    );
I2C_BIT_i_35: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFEEEA"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(3),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => Slave_Addr(4),
      I3 => Slave_Addr(0),
      I4 => LUT_DATA(0),
      O => I2C_BIT_i_35_n_0
    );
I2C_BIT_i_36: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00000000E7FF0000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(3),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => I2C_BIT_i_36_n_0
    );
I2C_BIT_i_37: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00000000EFDF0000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(3),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => I2C_BIT_i_37_n_0
    );
I2C_BIT_i_5: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B8BBB888"
    )
        port map (
      I0 => I2C_BIT_i_12_n_0,
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => I2C_BIT_i_13_n_0,
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => I2C_BIT_i_14_n_0,
      O => I2C_BIT_i_5_n_0
    );
I2C_BIT_i_6: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FE08"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(3),
      O => I2C_BIT_i_6_n_0
    );
I2C_BIT_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => I2C_BIT_i_1_n_0,
      PRE => rst,
      Q => I2C_BIT_reg_n_0
    );
I2C_BIT_reg_i_15: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_23_n_0,
      I1 => I2C_BIT_i_24_n_0,
      O => I2C_BIT_reg_i_15_n_0,
      S => \SD_COUNTER_reg__0\(0)
    );
I2C_BIT_reg_i_16: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_25_n_0,
      I1 => I2C_BIT_i_26_n_0,
      O => I2C_BIT_reg_i_16_n_0,
      S => \SD_COUNTER_reg__0\(0)
    );
I2C_BIT_reg_i_21: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_36_n_0,
      I1 => I2C_BIT_i_37_n_0,
      O => I2C_BIT_reg_i_21_n_0,
      S => \SD_COUNTER_reg__0\(5)
    );
I2C_BIT_reg_i_3: unisim.vcomponents.MUXF8
     port map (
      I0 => I2C_BIT_reg_i_8_n_0,
      I1 => I2C_BIT_reg_i_9_n_0,
      O => I2C_BIT_reg_i_3_n_0,
      S => mI2C_WR_reg
    );
I2C_BIT_reg_i_4: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_10_n_0,
      I1 => I2C_BIT_i_11_n_0,
      O => I2C_BIT6_out,
      S => \SD_COUNTER_reg__0\(4)
    );
I2C_BIT_reg_i_7: unisim.vcomponents.MUXF8
     port map (
      I0 => I2C_BIT_reg_i_15_n_0,
      I1 => I2C_BIT_reg_i_16_n_0,
      O => I2C_BIT_reg_i_7_n_0,
      S => \SD_COUNTER_reg__0\(2)
    );
I2C_BIT_reg_i_8: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_17_n_0,
      I1 => I2C_BIT_i_18_n_0,
      O => I2C_BIT_reg_i_8_n_0,
      S => \SD_COUNTER_reg__0\(5)
    );
I2C_BIT_reg_i_9: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_BIT_i_19_n_0,
      I1 => I2C_BIT_i_20_n_0,
      O => I2C_BIT_reg_i_9_n_0,
      S => \SD_COUNTER_reg__0\(5)
    );
I2C_SCLK_INST_0: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B8FFB800"
    )
        port map (
      I0 => I2C_SCLK_INST_0_i_1_n_0,
      I1 => \SD_COUNTER_reg__0\(4),
      I2 => I2C_SCLK_INST_0_i_2_n_0,
      I3 => mI2C_GO_reg,
      I4 => SCLK_reg_n_0,
      O => I2C_SCLK
    );
I2C_SCLK_INST_0_i_1: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B8BBB888"
    )
        port map (
      I0 => I2C_SCLK_INST_0_i_3_n_0,
      I1 => mI2C_WR_reg,
      I2 => I2C_SCLK_INST_0_i_4_n_0,
      I3 => \SD_COUNTER_reg__0\(5),
      I4 => I2C_SCLK_INST_0_i_5_n_0,
      O => I2C_SCLK_INST_0_i_1_n_0
    );
I2C_SCLK_INST_0_i_2: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B8FFB800"
    )
        port map (
      I0 => I2C_SCLK_INST_0_i_6_n_0,
      I1 => mI2C_WR_reg,
      I2 => I2C_SCLK_INST_0_i_7_n_0,
      I3 => \SD_COUNTER_reg__0\(5),
      I4 => I2C_SCLK_INST_0_i_8_n_0,
      O => I2C_SCLK_INST_0_i_2_n_0
    );
I2C_SCLK_INST_0_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FF54FF55AB00AA00"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => SCLK_reg_n_0,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => mI2C_CTRL_CLK,
      O => I2C_SCLK_INST_0_i_3_n_0
    );
I2C_SCLK_INST_0_i_4: unisim.vcomponents.LUT5
    generic map(
      INIT => X"F0F7F080"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => SCLK_reg_n_0,
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => mI2C_CTRL_CLK,
      O => I2C_SCLK_INST_0_i_4_n_0
    );
I2C_SCLK_INST_0_i_5: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FF10FFFFEF000000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => SCLK_reg_n_0,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => mI2C_CTRL_CLK,
      O => I2C_SCLK_INST_0_i_5_n_0
    );
I2C_SCLK_INST_0_i_6: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FF00FF17FF00E800"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => SCLK_reg_n_0,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => mI2C_CTRL_CLK,
      O => I2C_SCLK_INST_0_i_6_n_0
    );
I2C_SCLK_INST_0_i_7: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FF46FF01B900FE00"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => mI2C_CTRL_CLK,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => SCLK_reg_n_0,
      O => I2C_SCLK_INST_0_i_7_n_0
    );
I2C_SCLK_INST_0_i_8: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FFA0FF1F5F00E000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => mI2C_CTRL_CLK,
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => SCLK_reg_n_0,
      O => I2C_SCLK_INST_0_i_8_n_0
    );
I2C_SDAT_INST_0: unisim.vcomponents.LUT2
    generic map(
      INIT => X"8"
    )
        port map (
      I0 => I2C_BIT_reg_n_0,
      I1 => SDO,
      O => I2C_SDAT
    );
I2C_SDAT_INST_0_i_1: unisim.vcomponents.MUXF7
     port map (
      I0 => I2C_SDAT_INST_0_i_2_n_0,
      I1 => I2C_SDAT_INST_0_i_3_n_0,
      O => SDO,
      S => \SD_COUNTER_reg__0\(4)
    );
I2C_SDAT_INST_0_i_2: unisim.vcomponents.LUT6
    generic map(
      INIT => X"B8FF8BF7FB7FBFFF"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(2),
      O => I2C_SDAT_INST_0_i_2_n_0
    );
I2C_SDAT_INST_0_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FFFFFFEFFFAFF0AF"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(5),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(1),
      I5 => \SD_COUNTER_reg__0\(2),
      O => I2C_SDAT_INST_0_i_3_n_0
    );
\LUT_INDEX[7]_i_4\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"2"
    )
        port map (
      I0 => i2c_en_r1,
      I1 => i2c_en_r0,
      O => \^sd_counter_reg[0]_0\
    );
SCLK_i_1: unisim.vcomponents.LUT5
    generic map(
      INIT => X"BABF8A80"
    )
        port map (
      I0 => SCLK_i_2_n_0,
      I1 => SCLK_i_3_n_0,
      I2 => \SD_COUNTER_reg__0\(4),
      I3 => SCLK_i_4_n_0,
      I4 => SCLK_reg_n_0,
      O => SCLK_i_1_n_0
    );
SCLK_i_2: unisim.vcomponents.LUT5
    generic map(
      INIT => X"EFE0FFFF"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => SCLK_i_5_n_0,
      I2 => \SD_COUNTER_reg__0\(4),
      I3 => SCLK_i_6_n_0,
      I4 => mI2C_GO_reg,
      O => SCLK_i_2_n_0
    );
SCLK_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"30003000B8FF3000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => mI2C_WR_reg,
      I2 => SCLK_i_7_n_0,
      I3 => mI2C_GO_reg,
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => SCLK_i_3_n_0
    );
SCLK_i_4: unisim.vcomponents.LUT6
    generic map(
      INIT => X"88F0FFFF88F00000"
    )
        port map (
      I0 => SCLK_i_8_n_0,
      I1 => mI2C_WR_reg,
      I2 => SCLK_i_9_n_0,
      I3 => \SD_COUNTER_reg__0\(5),
      I4 => mI2C_GO_reg,
      I5 => \^sd_counter_reg[0]_0\,
      O => SCLK_i_4_n_0
    );
SCLK_i_5: unisim.vcomponents.LUT5
    generic map(
      INIT => X"B0A0FFFF"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => \SD_COUNTER_reg__0\(3),
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => \SD_COUNTER_reg__0\(1),
      O => SCLK_i_5_n_0
    );
SCLK_i_6: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EFA0EEA0FF5FFF5F"
    )
        port map (
      I0 => mI2C_WR_reg,
      I1 => \SD_COUNTER_reg__0\(5),
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(1),
      O => SCLK_i_6_n_0
    );
SCLK_i_7: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EF800000E2000000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(5),
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \^sd_counter_reg[0]_0\,
      I5 => \SD_COUNTER_reg__0\(1),
      O => SCLK_i_7_n_0
    );
SCLK_i_8: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00FF000000600000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => i2c_en_r0,
      I4 => i2c_en_r1,
      I5 => \SD_COUNTER_reg__0\(3),
      O => SCLK_i_8_n_0
    );
SCLK_i_9: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000000300010"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(1),
      I1 => i2c_en_r0,
      I2 => i2c_en_r1,
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(0),
      I5 => \SD_COUNTER_reg__0\(2),
      O => SCLK_i_9_n_0
    );
SCLK_reg: unisim.vcomponents.FDPE
     port map (
      C => iCLK,
      CE => '1',
      D => SCLK_i_1_n_0,
      PRE => rst,
      Q => SCLK_reg_n_0
    );
\SD_COUNTER[0]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"10"
    )
        port map (
      I0 => mI2C_END,
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => mI2C_GO_reg,
      O => \p_0_in__0\(0)
    );
\SD_COUNTER[1]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"1400"
    )
        port map (
      I0 => mI2C_END,
      I1 => \SD_COUNTER_reg__0\(0),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => mI2C_GO_reg,
      O => \p_0_in__0\(1)
    );
\SD_COUNTER[2]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"14440000"
    )
        port map (
      I0 => mI2C_END,
      I1 => \SD_COUNTER_reg__0\(2),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => \SD_COUNTER_reg__0\(1),
      I4 => mI2C_GO_reg,
      O => \p_0_in__0\(2)
    );
\SD_COUNTER[3]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"1555400000000000"
    )
        port map (
      I0 => mI2C_END,
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => \SD_COUNTER_reg__0\(2),
      I4 => \SD_COUNTER_reg__0\(3),
      I5 => mI2C_GO_reg,
      O => \p_0_in__0\(3)
    );
\SD_COUNTER[4]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"2212222200000000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(4),
      I1 => mI2C_END,
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => \SD_COUNTER[4]_i_2_n_0\,
      I4 => \SD_COUNTER_reg__0\(2),
      I5 => mI2C_GO_reg,
      O => \p_0_in__0\(4)
    );
\SD_COUNTER[4]_i_2\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"7"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(3),
      I1 => \SD_COUNTER_reg__0\(1),
      O => \SD_COUNTER[4]_i_2_n_0\
    );
\SD_COUNTER[5]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"EFFFFFFF40000000"
    )
        port map (
      I0 => mI2C_END,
      I1 => \SD_COUNTER[5]_i_3_n_0\,
      I2 => \SD_COUNTER_reg__0\(5),
      I3 => mI2C_GO_reg,
      I4 => \SD_COUNTER_reg__0\(4),
      I5 => \^sd_counter_reg[0]_0\,
      O => SD_COUNTER
    );
\SD_COUNTER[5]_i_2\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"8B880000"
    )
        port map (
      I0 => \SD_COUNTER[5]_i_4_n_0\,
      I1 => \SD_COUNTER_reg__0\(4),
      I2 => mI2C_END,
      I3 => \SD_COUNTER_reg__0\(5),
      I4 => mI2C_GO_reg,
      O => \p_0_in__0\(5)
    );
\SD_COUNTER[5]_i_3\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"000000007FFF0000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(3),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(0),
      I3 => \SD_COUNTER_reg__0\(2),
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => \SD_COUNTER[5]_i_3_n_0\
    );
\SD_COUNTER[5]_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"000000007FFF8000"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(2),
      I1 => \SD_COUNTER_reg__0\(3),
      I2 => \SD_COUNTER_reg__0\(1),
      I3 => \SD_COUNTER_reg__0\(0),
      I4 => \SD_COUNTER_reg__0\(5),
      I5 => mI2C_END,
      O => \SD_COUNTER[5]_i_4_n_0\
    );
\SD_COUNTER_reg[0]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(0),
      Q => \SD_COUNTER_reg__0\(0)
    );
\SD_COUNTER_reg[1]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(1),
      Q => \SD_COUNTER_reg__0\(1)
    );
\SD_COUNTER_reg[2]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(2),
      Q => \SD_COUNTER_reg__0\(2)
    );
\SD_COUNTER_reg[3]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(3),
      Q => \SD_COUNTER_reg__0\(3)
    );
\SD_COUNTER_reg[4]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(4),
      Q => \SD_COUNTER_reg__0\(4)
    );
\SD_COUNTER_reg[5]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => SD_COUNTER,
      CLR => rst,
      D => \p_0_in__0\(5),
      Q => \SD_COUNTER_reg__0\(5)
    );
g0_b0: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FEE01DFF76FFDFF7"
    )
        port map (
      I0 => \SD_COUNTER_reg__0\(0),
      I1 => \SD_COUNTER_reg__0\(1),
      I2 => \SD_COUNTER_reg__0\(2),
      I3 => \SD_COUNTER_reg__0\(3),
      I4 => \SD_COUNTER_reg__0\(4),
      I5 => \SD_COUNTER_reg__0\(5),
      O => g0_b0_n_0
    );
mI2C_WR_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"B000B000F4FFB000"
    )
        port map (
      I0 => Q(1),
      I1 => mI2C_END,
      I2 => \LUT_INDEX_reg[5]\,
      I3 => Q(0),
      I4 => i2c_en_r1,
      I5 => i2c_en_r0,
      O => E(0)
    );
\mSetup_ST[0]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"00000051"
    )
        port map (
      I0 => Q(1),
      I1 => \LUT_INDEX_reg[7]\(6),
      I2 => \LUT_INDEX_reg[5]_0\,
      I3 => mI2C_END,
      I4 => Q(0),
      O => D(0)
    );
\mSetup_ST[1]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"2E220000"
    )
        port map (
      I0 => \mSetup_ST[1]_i_2_n_0\,
      I1 => \LUT_INDEX_reg[7]\(6),
      I2 => \LUT_INDEX_reg[7]\(5),
      I3 => \mSetup_ST[1]_i_3_n_0\,
      I4 => Q(0),
      O => D(1)
    );
\mSetup_ST[1]_i_2\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"01FF0100"
    )
        port map (
      I0 => ACKW1_reg_n_0,
      I1 => ACKW2_reg_n_0,
      I2 => ACKW3_reg_n_0,
      I3 => mI2C_WR_reg,
      I4 => \mSetup_ST[1]_i_4_n_0\,
      O => \mSetup_ST[1]_i_2_n_0\
    );
\mSetup_ST[1]_i_3\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0057FFFF00000000"
    )
        port map (
      I0 => \LUT_INDEX_reg[7]\(2),
      I1 => \LUT_INDEX_reg[7]\(1),
      I2 => \LUT_INDEX_reg[7]\(0),
      I3 => \LUT_INDEX_reg[7]\(3),
      I4 => \LUT_INDEX_reg[7]\(4),
      I5 => \mSetup_ST[1]_i_2_n_0\,
      O => \mSetup_ST[1]_i_3_n_0\
    );
\mSetup_ST[1]_i_4\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"01"
    )
        port map (
      I0 => ACKR1_reg_n_0,
      I1 => ACKR2_reg_n_0,
      I2 => ACKR3_reg_n_0,
      O => \mSetup_ST[1]_i_4_n_0\
    );
end STRUCTURE;
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_IICctrl is
  port (
    LUT_INDEX : out STD_LOGIC_VECTOR ( 7 downto 0 );
    I2C_SCLK : out STD_LOGIC;
    I2C_SDAT : inout STD_LOGIC;
    iCLK : in STD_LOGIC;
    rst : in STD_LOGIC;
    Slave_Addr : in STD_LOGIC_VECTOR ( 7 downto 0 );
    LUT_DATA : in STD_LOGIC_VECTOR ( 15 downto 0 )
  );
end decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_IICctrl;

architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_IICctrl is
  signal \^lut_index\ : STD_LOGIC_VECTOR ( 7 downto 0 );
  signal \LUT_INDEX[6]_i_2_n_0\ : STD_LOGIC;
  signal \LUT_INDEX[7]_i_1_n_0\ : STD_LOGIC;
  signal \LUT_INDEX[7]_i_3_n_0\ : STD_LOGIC;
  signal \LUT_INDEX[7]_i_5_n_0\ : STD_LOGIC;
  signal \LUT_INDEX[7]_i_6_n_0\ : STD_LOGIC;
  signal i2c_en_r0 : STD_LOGIC;
  signal i2c_en_r1 : STD_LOGIC;
  signal mI2C_CLK_DIV : STD_LOGIC_VECTOR ( 10 downto 0 );
  signal \mI2C_CLK_DIV[10]_i_2_n_0\ : STD_LOGIC;
  signal \mI2C_CLK_DIV[10]_i_3_n_0\ : STD_LOGIC;
  signal \mI2C_CLK_DIV[10]_i_4_n_0\ : STD_LOGIC;
  signal \mI2C_CLK_DIV[5]_i_2_n_0\ : STD_LOGIC;
  signal \mI2C_CLK_DIV[9]_i_2_n_0\ : STD_LOGIC;
  signal mI2C_CTRL_CLK : STD_LOGIC;
  signal mI2C_CTRL_CLK_i_1_n_0 : STD_LOGIC;
  signal mI2C_GO0_out : STD_LOGIC;
  signal mI2C_GO2_out : STD_LOGIC;
  signal mI2C_GO_reg_n_0 : STD_LOGIC;
  signal mI2C_WR4_out : STD_LOGIC;
  signal mI2C_WR_i_3_n_0 : STD_LOGIC;
  signal mI2C_WR_i_4_n_0 : STD_LOGIC;
  signal mI2C_WR_i_5_n_0 : STD_LOGIC;
  signal mI2C_WR_i_6_n_0 : STD_LOGIC;
  signal mI2C_WR_reg_n_0 : STD_LOGIC;
  signal \mSetup_ST_reg_n_0_[0]\ : STD_LOGIC;
  signal \mSetup_ST_reg_n_0_[1]\ : STD_LOGIC;
  signal p_0_in : STD_LOGIC_VECTOR ( 10 downto 0 );
  signal \p_0_in__0__0\ : STD_LOGIC_VECTOR ( 7 downto 0 );
  signal u_I2C_Controller_n_1 : STD_LOGIC;
  signal u_I2C_Controller_n_2 : STD_LOGIC;
  signal u_I2C_Controller_n_3 : STD_LOGIC;
  attribute SOFT_HLUTNM : string;
  attribute SOFT_HLUTNM of \LUT_INDEX[0]_i_1\ : label is "soft_lutpair13";
  attribute SOFT_HLUTNM of \LUT_INDEX[1]_i_1\ : label is "soft_lutpair13";
  attribute SOFT_HLUTNM of \LUT_INDEX[2]_i_1\ : label is "soft_lutpair12";
  attribute SOFT_HLUTNM of \LUT_INDEX[3]_i_1\ : label is "soft_lutpair5";
  attribute SOFT_HLUTNM of \LUT_INDEX[4]_i_1\ : label is "soft_lutpair5";
  attribute SOFT_HLUTNM of \LUT_INDEX[6]_i_2\ : label is "soft_lutpair12";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[0]_i_1\ : label is "soft_lutpair11";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[10]_i_1\ : label is "soft_lutpair10";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[10]_i_2\ : label is "soft_lutpair9";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[2]_i_1\ : label is "soft_lutpair8";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[3]_i_1\ : label is "soft_lutpair8";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[5]_i_1\ : label is "soft_lutpair11";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[6]_i_1\ : label is "soft_lutpair9";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[7]_i_1\ : label is "soft_lutpair7";
  attribute SOFT_HLUTNM of \mI2C_CLK_DIV[8]_i_1\ : label is "soft_lutpair7";
  attribute SOFT_HLUTNM of mI2C_CTRL_CLK_i_1 : label is "soft_lutpair10";
  attribute SOFT_HLUTNM of mI2C_GO_i_1 : label is "soft_lutpair6";
  attribute SOFT_HLUTNM of mI2C_WR_i_2 : label is "soft_lutpair6";
begin
  LUT_INDEX(7 downto 0) <= \^lut_index\(7 downto 0);
\LUT_INDEX[0]_i_1\: unisim.vcomponents.LUT1
    generic map(
      INIT => X"1"
    )
        port map (
      I0 => \^lut_index\(0),
      O => \p_0_in__0__0\(0)
    );
\LUT_INDEX[1]_i_1\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"6"
    )
        port map (
      I0 => \^lut_index\(1),
      I1 => \^lut_index\(0),
      O => \p_0_in__0__0\(1)
    );
\LUT_INDEX[2]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"6A"
    )
        port map (
      I0 => \^lut_index\(2),
      I1 => \^lut_index\(1),
      I2 => \^lut_index\(0),
      O => \p_0_in__0__0\(2)
    );
\LUT_INDEX[3]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"6AAA"
    )
        port map (
      I0 => \^lut_index\(3),
      I1 => \^lut_index\(1),
      I2 => \^lut_index\(0),
      I3 => \^lut_index\(2),
      O => \p_0_in__0__0\(3)
    );
\LUT_INDEX[4]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"6AAAAAAA"
    )
        port map (
      I0 => \^lut_index\(4),
      I1 => \^lut_index\(2),
      I2 => \^lut_index\(0),
      I3 => \^lut_index\(1),
      I4 => \^lut_index\(3),
      O => \p_0_in__0__0\(4)
    );
\LUT_INDEX[5]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"6AAAAAAAAAAAAAAA"
    )
        port map (
      I0 => \^lut_index\(5),
      I1 => \^lut_index\(3),
      I2 => \^lut_index\(1),
      I3 => \^lut_index\(0),
      I4 => \^lut_index\(2),
      I5 => \^lut_index\(4),
      O => \p_0_in__0__0\(5)
    );
\LUT_INDEX[6]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"6AAAAAAAAAAAAAAA"
    )
        port map (
      I0 => \^lut_index\(6),
      I1 => \^lut_index\(4),
      I2 => \^lut_index\(2),
      I3 => \LUT_INDEX[6]_i_2_n_0\,
      I4 => \^lut_index\(3),
      I5 => \^lut_index\(5),
      O => \p_0_in__0__0\(6)
    );
\LUT_INDEX[6]_i_2\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"8"
    )
        port map (
      I0 => \^lut_index\(0),
      I1 => \^lut_index\(1),
      O => \LUT_INDEX[6]_i_2_n_0\
    );
\LUT_INDEX[7]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0000000008AA0800"
    )
        port map (
      I0 => \mSetup_ST_reg_n_0_[1]\,
      I1 => \LUT_INDEX[7]_i_3_n_0\,
      I2 => \^lut_index\(6),
      I3 => \^lut_index\(7),
      I4 => u_I2C_Controller_n_1,
      I5 => \mSetup_ST_reg_n_0_[0]\,
      O => \LUT_INDEX[7]_i_1_n_0\
    );
\LUT_INDEX[7]_i_2\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"B8CC"
    )
        port map (
      I0 => \LUT_INDEX[7]_i_5_n_0\,
      I1 => \^lut_index\(7),
      I2 => \LUT_INDEX[7]_i_6_n_0\,
      I3 => \^lut_index\(6),
      O => \p_0_in__0__0\(7)
    );
\LUT_INDEX[7]_i_3\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"0057FFFF00000000"
    )
        port map (
      I0 => \^lut_index\(3),
      I1 => \^lut_index\(2),
      I2 => \^lut_index\(1),
      I3 => \^lut_index\(4),
      I4 => \^lut_index\(5),
      I5 => u_I2C_Controller_n_1,
      O => \LUT_INDEX[7]_i_3_n_0\
    );
\LUT_INDEX[7]_i_5\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"7FFFFFFFFFFFFFFF"
    )
        port map (
      I0 => \^lut_index\(4),
      I1 => \^lut_index\(2),
      I2 => \^lut_index\(1),
      I3 => \^lut_index\(0),
      I4 => \^lut_index\(3),
      I5 => \^lut_index\(5),
      O => \LUT_INDEX[7]_i_5_n_0\
    );
\LUT_INDEX[7]_i_6\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"8000000000000000"
    )
        port map (
      I0 => \^lut_index\(4),
      I1 => \^lut_index\(2),
      I2 => \^lut_index\(0),
      I3 => \^lut_index\(1),
      I4 => \^lut_index\(3),
      I5 => \^lut_index\(5),
      O => \LUT_INDEX[7]_i_6_n_0\
    );
\LUT_INDEX_reg[0]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(0),
      Q => \^lut_index\(0)
    );
\LUT_INDEX_reg[1]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(1),
      Q => \^lut_index\(1)
    );
\LUT_INDEX_reg[2]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(2),
      Q => \^lut_index\(2)
    );
\LUT_INDEX_reg[3]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(3),
      Q => \^lut_index\(3)
    );
\LUT_INDEX_reg[4]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(4),
      Q => \^lut_index\(4)
    );
\LUT_INDEX_reg[5]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(5),
      Q => \^lut_index\(5)
    );
\LUT_INDEX_reg[6]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(6),
      Q => \^lut_index\(6)
    );
\LUT_INDEX_reg[7]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => \LUT_INDEX[7]_i_1_n_0\,
      CLR => rst,
      D => \p_0_in__0__0\(7),
      Q => \^lut_index\(7)
    );
i2c_en_r0_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => mI2C_CTRL_CLK,
      Q => i2c_en_r0
    );
i2c_en_r1_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => i2c_en_r0,
      Q => i2c_en_r1
    );
\mI2C_CLK_DIV[0]_i_1\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"2"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CLK_DIV(0),
      O => p_0_in(0)
    );
\mI2C_CLK_DIV[10]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"7080"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_2_n_0\,
      I1 => mI2C_CLK_DIV(9),
      I2 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I3 => mI2C_CLK_DIV(10),
      O => p_0_in(10)
    );
\mI2C_CLK_DIV[10]_i_2\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"0080"
    )
        port map (
      I0 => mI2C_CLK_DIV(8),
      I1 => mI2C_CLK_DIV(7),
      I2 => mI2C_CLK_DIV(6),
      I3 => \mI2C_CLK_DIV[9]_i_2_n_0\,
      O => \mI2C_CLK_DIV[10]_i_2_n_0\
    );
\mI2C_CLK_DIV[10]_i_3\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"1F001F1F1F1F1F1F"
    )
        port map (
      I0 => mI2C_CLK_DIV(8),
      I1 => mI2C_CLK_DIV(9),
      I2 => mI2C_CLK_DIV(10),
      I3 => \mI2C_CLK_DIV[10]_i_4_n_0\,
      I4 => mI2C_CLK_DIV(6),
      I5 => mI2C_CLK_DIV(7),
      O => \mI2C_CLK_DIV[10]_i_3_n_0\
    );
\mI2C_CLK_DIV[10]_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"777777777777777F"
    )
        port map (
      I0 => mI2C_CLK_DIV(5),
      I1 => mI2C_CLK_DIV(10),
      I2 => mI2C_CLK_DIV(3),
      I3 => mI2C_CLK_DIV(4),
      I4 => mI2C_CLK_DIV(1),
      I5 => mI2C_CLK_DIV(2),
      O => \mI2C_CLK_DIV[10]_i_4_n_0\
    );
\mI2C_CLK_DIV[1]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"28"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CLK_DIV(0),
      I2 => mI2C_CLK_DIV(1),
      O => p_0_in(1)
    );
\mI2C_CLK_DIV[2]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"2A80"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CLK_DIV(1),
      I2 => mI2C_CLK_DIV(0),
      I3 => mI2C_CLK_DIV(2),
      O => p_0_in(2)
    );
\mI2C_CLK_DIV[3]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"2AAA8000"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CLK_DIV(0),
      I2 => mI2C_CLK_DIV(1),
      I3 => mI2C_CLK_DIV(2),
      I4 => mI2C_CLK_DIV(3),
      O => p_0_in(3)
    );
\mI2C_CLK_DIV[4]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"2AAAAAAA80000000"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CLK_DIV(3),
      I2 => mI2C_CLK_DIV(2),
      I3 => mI2C_CLK_DIV(1),
      I4 => mI2C_CLK_DIV(0),
      I5 => mI2C_CLK_DIV(4),
      O => p_0_in(4)
    );
\mI2C_CLK_DIV[5]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"28"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => \mI2C_CLK_DIV[5]_i_2_n_0\,
      I2 => mI2C_CLK_DIV(5),
      O => p_0_in(5)
    );
\mI2C_CLK_DIV[5]_i_2\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"80000000"
    )
        port map (
      I0 => mI2C_CLK_DIV(4),
      I1 => mI2C_CLK_DIV(3),
      I2 => mI2C_CLK_DIV(2),
      I3 => mI2C_CLK_DIV(1),
      I4 => mI2C_CLK_DIV(0),
      O => \mI2C_CLK_DIV[5]_i_2_n_0\
    );
\mI2C_CLK_DIV[6]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"82"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => \mI2C_CLK_DIV[9]_i_2_n_0\,
      I2 => mI2C_CLK_DIV(6),
      O => p_0_in(6)
    );
\mI2C_CLK_DIV[7]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"D020"
    )
        port map (
      I0 => mI2C_CLK_DIV(6),
      I1 => \mI2C_CLK_DIV[9]_i_2_n_0\,
      I2 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I3 => mI2C_CLK_DIV(7),
      O => p_0_in(7)
    );
\mI2C_CLK_DIV[8]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"BF004000"
    )
        port map (
      I0 => \mI2C_CLK_DIV[9]_i_2_n_0\,
      I1 => mI2C_CLK_DIV(6),
      I2 => mI2C_CLK_DIV(7),
      I3 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I4 => mI2C_CLK_DIV(8),
      O => p_0_in(8)
    );
\mI2C_CLK_DIV[9]_i_1\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FF7F000000800000"
    )
        port map (
      I0 => mI2C_CLK_DIV(8),
      I1 => mI2C_CLK_DIV(7),
      I2 => mI2C_CLK_DIV(6),
      I3 => \mI2C_CLK_DIV[9]_i_2_n_0\,
      I4 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I5 => mI2C_CLK_DIV(9),
      O => p_0_in(9)
    );
\mI2C_CLK_DIV[9]_i_2\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"7FFFFFFFFFFFFFFF"
    )
        port map (
      I0 => mI2C_CLK_DIV(0),
      I1 => mI2C_CLK_DIV(1),
      I2 => mI2C_CLK_DIV(2),
      I3 => mI2C_CLK_DIV(3),
      I4 => mI2C_CLK_DIV(4),
      I5 => mI2C_CLK_DIV(5),
      O => \mI2C_CLK_DIV[9]_i_2_n_0\
    );
\mI2C_CLK_DIV_reg[0]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(0),
      Q => mI2C_CLK_DIV(0)
    );
\mI2C_CLK_DIV_reg[10]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(10),
      Q => mI2C_CLK_DIV(10)
    );
\mI2C_CLK_DIV_reg[1]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(1),
      Q => mI2C_CLK_DIV(1)
    );
\mI2C_CLK_DIV_reg[2]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(2),
      Q => mI2C_CLK_DIV(2)
    );
\mI2C_CLK_DIV_reg[3]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(3),
      Q => mI2C_CLK_DIV(3)
    );
\mI2C_CLK_DIV_reg[4]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(4),
      Q => mI2C_CLK_DIV(4)
    );
\mI2C_CLK_DIV_reg[5]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(5),
      Q => mI2C_CLK_DIV(5)
    );
\mI2C_CLK_DIV_reg[6]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(6),
      Q => mI2C_CLK_DIV(6)
    );
\mI2C_CLK_DIV_reg[7]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(7),
      Q => mI2C_CLK_DIV(7)
    );
\mI2C_CLK_DIV_reg[8]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(8),
      Q => mI2C_CLK_DIV(8)
    );
\mI2C_CLK_DIV_reg[9]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => p_0_in(9),
      Q => mI2C_CLK_DIV(9)
    );
mI2C_CTRL_CLK_i_1: unisim.vcomponents.LUT2
    generic map(
      INIT => X"9"
    )
        port map (
      I0 => \mI2C_CLK_DIV[10]_i_3_n_0\,
      I1 => mI2C_CTRL_CLK,
      O => mI2C_CTRL_CLK_i_1_n_0
    );
mI2C_CTRL_CLK_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => '1',
      CLR => rst,
      D => mI2C_CTRL_CLK_i_1_n_0,
      Q => mI2C_CTRL_CLK
    );
mI2C_GO_i_1: unisim.vcomponents.LUT4
    generic map(
      INIT => X"0045"
    )
        port map (
      I0 => \mSetup_ST_reg_n_0_[1]\,
      I1 => mI2C_WR_i_4_n_0,
      I2 => \^lut_index\(7),
      I3 => \mSetup_ST_reg_n_0_[0]\,
      O => mI2C_GO0_out
    );
mI2C_GO_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => mI2C_GO2_out,
      CLR => rst,
      D => mI2C_GO0_out,
      Q => mI2C_GO_reg_n_0
    );
mI2C_WR_i_2: unisim.vcomponents.LUT5
    generic map(
      INIT => X"00004540"
    )
        port map (
      I0 => \mSetup_ST_reg_n_0_[1]\,
      I1 => mI2C_WR_i_4_n_0,
      I2 => \^lut_index\(7),
      I3 => mI2C_WR_i_5_n_0,
      I4 => \mSetup_ST_reg_n_0_[0]\,
      O => mI2C_WR4_out
    );
mI2C_WR_i_3: unisim.vcomponents.LUT6
    generic map(
      INIT => X"08F8080800000000"
    )
        port map (
      I0 => \^lut_index\(5),
      I1 => mI2C_WR_i_6_n_0,
      I2 => \^lut_index\(6),
      I3 => i2c_en_r0,
      I4 => i2c_en_r1,
      I5 => \^lut_index\(7),
      O => mI2C_WR_i_3_n_0
    );
mI2C_WR_i_4: unisim.vcomponents.LUT6
    generic map(
      INIT => X"000000005555777F"
    )
        port map (
      I0 => \^lut_index\(5),
      I1 => \^lut_index\(3),
      I2 => \^lut_index\(1),
      I3 => \^lut_index\(2),
      I4 => \^lut_index\(4),
      I5 => \^lut_index\(6),
      O => mI2C_WR_i_4_n_0
    );
mI2C_WR_i_5: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FFFFFFFFFFFFFFFE"
    )
        port map (
      I0 => \^lut_index\(5),
      I1 => \^lut_index\(3),
      I2 => \^lut_index\(2),
      I3 => \^lut_index\(1),
      I4 => \^lut_index\(4),
      I5 => \^lut_index\(6),
      O => mI2C_WR_i_5_n_0
    );
mI2C_WR_i_6: unisim.vcomponents.LUT6
    generic map(
      INIT => X"00F000E000A000A0"
    )
        port map (
      I0 => \^lut_index\(4),
      I1 => \^lut_index\(1),
      I2 => i2c_en_r1,
      I3 => i2c_en_r0,
      I4 => \^lut_index\(2),
      I5 => \^lut_index\(3),
      O => mI2C_WR_i_6_n_0
    );
mI2C_WR_reg: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => mI2C_GO2_out,
      CLR => rst,
      D => mI2C_WR4_out,
      Q => mI2C_WR_reg_n_0
    );
\mSetup_ST_reg[0]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => mI2C_GO2_out,
      CLR => rst,
      D => u_I2C_Controller_n_3,
      Q => \mSetup_ST_reg_n_0_[0]\
    );
\mSetup_ST_reg[1]\: unisim.vcomponents.FDCE
     port map (
      C => iCLK,
      CE => mI2C_GO2_out,
      CLR => rst,
      D => u_I2C_Controller_n_2,
      Q => \mSetup_ST_reg_n_0_[1]\
    );
u_I2C_Controller: entity work.decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_I2C_Controller
     port map (
      D(1) => u_I2C_Controller_n_2,
      D(0) => u_I2C_Controller_n_3,
      E(0) => mI2C_GO2_out,
      I2C_SCLK => I2C_SCLK,
      I2C_SDAT => I2C_SDAT,
      LUT_DATA(15 downto 0) => LUT_DATA(15 downto 0),
      \LUT_INDEX_reg[5]\ => mI2C_WR_i_3_n_0,
      \LUT_INDEX_reg[5]_0\ => mI2C_WR_i_4_n_0,
      \LUT_INDEX_reg[7]\(6 downto 0) => \^lut_index\(7 downto 1),
      Q(1) => \mSetup_ST_reg_n_0_[1]\,
      Q(0) => \mSetup_ST_reg_n_0_[0]\,
      \SD_COUNTER_reg[0]_0\ => u_I2C_Controller_n_1,
      Slave_Addr(7 downto 0) => Slave_Addr(7 downto 0),
      i2c_en_r0 => i2c_en_r0,
      i2c_en_r1 => i2c_en_r1,
      iCLK => iCLK,
      mI2C_CTRL_CLK => mI2C_CTRL_CLK,
      mI2C_GO_reg => mI2C_GO_reg_n_0,
      mI2C_WR_reg => mI2C_WR_reg_n_0,
      rst => rst
    );
end STRUCTURE;
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix is
  port (
    iCLK : in STD_LOGIC;
    rst : in STD_LOGIC;
    I2C_SCLK : out STD_LOGIC;
    I2C_SDAT : inout STD_LOGIC;
    LUT_INDEX : out STD_LOGIC_VECTOR ( 7 downto 0 );
    LUT_DATA : in STD_LOGIC_VECTOR ( 15 downto 0 );
    Slave_Addr : in STD_LOGIC_VECTOR ( 7 downto 0 )
  );
  attribute NotValidForBitStream : boolean;
  attribute NotValidForBitStream of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix : entity is true;
  attribute CHECK_LICENSE_TYPE : string;
  attribute CHECK_LICENSE_TYPE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix : entity is "IICctrl_0,IICctrl,{}";
  attribute DowngradeIPIdentifiedWarnings : string;
  attribute DowngradeIPIdentifiedWarnings of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix : entity is "yes";
  attribute X_CORE_INFO : string;
  attribute X_CORE_INFO of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix : entity is "IICctrl,Vivado 2017.1";
end decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix;

architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix is
begin
inst: entity work.decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_IICctrl
     port map (
      I2C_SCLK => I2C_SCLK,
      I2C_SDAT => I2C_SDAT,
      LUT_DATA(15 downto 0) => LUT_DATA(15 downto 0),
      LUT_INDEX(7 downto 0) => LUT_INDEX(7 downto 0),
      Slave_Addr(7 downto 0) => Slave_Addr(7 downto 0),
      iCLK => iCLK,
      rst => rst
    );
end STRUCTURE;
